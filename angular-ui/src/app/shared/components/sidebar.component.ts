...
@Component({
  ...
  template: `
  <aside class="hidden md:flex w-64 bg-[#B71C1C] text-[#F8F7F2] flex-col shadow-md shrink-0 z-20 h-full relative" [class.flex]="!isCliente && !isMozo">
    ...
    <nav class="flex-1 py-8 px-4 flex flex-col gap-3 overflow-y-auto">
      @if (isCliente) {
    ...
    <div class="p-4 border-t border-red-700 flex flex-col gap-2 bg-red-950/10 shrink-0">
  ...
  <!-- Mobile Bottom Nav -->
  @if (isCliente || isMozo) {
    <div class="md:hidden fixed bottom-0 left-0 w-full bg-white border-t border-[#2E221B]/10 text-[#2E221B]/60 flex justify-around items-center z-[100] h-16 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
      @if (isCliente) {
        <a routerLink="/cliente" routerLinkActive="text-[#B71C1C] font-bold" [routerLinkActiveOptions]="{exact: true}" class="flex flex-col items-center p-2 flex-1 hover:text-[#B71C1C] transition-colors">
          <svg class="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
          <span class="text-[10px] uppercase tracking-wider">Menú</span>
        </a>
        <a routerLink="/cliente/mis-pedidos" routerLinkActive="text-[#B71C1C] font-bold" class="flex flex-col items-center p-2 flex-1 hover:text-[#B71C1C] transition-colors">
          <svg class="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>
          <span class="text-[10px] uppercase tracking-wider">Pedidos</span>
        </a>
      } @else if (isMozo) {
        <a routerLink="/mozo" routerLinkActive="text-[#B71C1C] font-bold" [routerLinkActiveOptions]="{exact: true}" class="flex flex-col items-center p-2 flex-1 hover:text-[#B71C1C] transition-colors">
          ...
          <span class="text-[10px] uppercase tracking-wider">Mesas</span>
        </a>
      }
      <button (click)="cerrarSesion()" class="flex flex-col items-center p-2 flex-1 hover:text-[#B71C1C] transition-colors">
        <svg class="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
        <span class="text-[10px] uppercase tracking-wider">Salir</span>
      </button>
    </div>
  }
  `
